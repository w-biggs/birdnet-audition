...
__name(displayPredictions, "displayPredictions");
function submitToBirdNET(path) {
  return __async(this, null, function* () {
    const fileBuffer = yield (0, import_promises.readFile)(path);
    const fileBlob = new Blob([fileBuffer], { type: "audio/wav" });
    const formData = new FormData();
    formData.append("upload", fileBlob, "file.wav");
    writeResult("Loading...");
    const response = yield fetch("https://birdnet.cornell.edu/api/upload", {
      method: "POST",
      body: formData
    });
